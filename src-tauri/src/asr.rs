...
impl AsrSession {
    ...
    pub fn append_audio(&self, base64_chunk: &str) {
        if !self.is_ready() || self.is_committed.load(std::sync::atomic::Ordering::SeqCst) {
            return;
        }
        let audio = match base64::engine::general_purpose::STANDARD.decode(base64_chunk) {
            Ok(data) => data,
            Err(_) => return,
        };
    ...
    pub async fn commit_and_await_final(&self) -> Result<String, String> {
        if !self.is_ready() {
...
pub async fn create_asr_session(
    ...
    headers.insert("X-Api-App-Key", connection.app_id.parse().unwrap());
    headers.insert("X-Api-Access-Key", connection.access_token.parse().unwrap());
    headers.insert("X-Api-Resource-Id", connection.resource_id.parse().unwrap());
    ...
    let is_committed = Arc::new(std::sync::atomic::AtomicBool::new(false));
        ...
        while let Some(msg) = stream.next().await {
            match msg {
                Ok(Message::Binary(data)) => {
                    ...
                    if let Some(payload) = parse_server_response(buffer) {
                        ...
                        if let Some(result) = payload.get("result") {
                            ...
                            let utterances = result.get("utterances").and_then(|v| v.as_array());
                ...
                Ok(Message::Close(frame)) => {
                    ...
                    if is_committed.load(std::sync::atomic::Ordering::SeqCst) {
