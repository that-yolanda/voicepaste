...
mod macos {
    ...
    struct Model {
        ...
        glass_mode: String, // "auto" | "light" | "dark" — only applied for vibrancy
        loaded: bool,
    }
        ...
        MODEL.with(|m| {
            ...
            match kind {
                ...
                "appearance" => {
                    ...
                    if let Some(g) = payload.and_then(|p| p.get("glass")).and_then(|v| v.as_str()) {
                        model.glass_mode = g.into();
                    }
                }
                _ => {}
            }
    ...
    fn ensure_loaded(app: &AppHandle, model: &mut Model) {
        ...
        if let Some(inner) = app.try_state::<std::sync::Arc<crate::app_state::AppInner>>() {
            if let Ok(config) = inner.config_manager.load_config() {
                ...
                if !config.app.overlay_glass_mode.is_empty() {
                    model.glass_mode = config.app.overlay_glass_mode.clone();
                }
            }
        }
    }
    ...
    /// The vibrancy (`NSVisualEffectView`) material does respond to `NSAppearance`, so
    /// for that style we honor the user's `overlay_glass_mode`: auto → "" (follow system),
    /// light → "light", dark → "dark".
    fn resolved_variant(model: &Model) -> &'static str {
        if uses_glass(&model.style) {
            return "";
        }
        match model.glass_mode.as_str() {
            "light" => "light",
            "dark" => "dark",
            _ => "",
        }
