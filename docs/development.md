...
- **Rust**: Tests use `#[path = "tests/xxx.rs"]` attribute, files under `src-tauri/src/asr/tests/`. Run with `cargo test`. Dev-dependencies include `tempfile` for isolated file I/O and `wiremock` for HTTP mocking.
- **Frontend**: Vitest + jsdom. Tests under `web/tests/`, helpers under `web/tests/helpers/`. `window.__TAURI__`, `window.voiceOverlay`, `window.voiceSettings`, and Web APIs are mocked via helper files.
